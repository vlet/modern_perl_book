...
Когда вы запустите программу, ничего заметного не произойдёт. Perl вызовет
...
зачастую содержащемуся в первом или доступному каким-либо иным способом.
...
методу найти созданное замыкание (и обойти C<AUTOLOAD()>). Этот код наконец
...
C<AUTOLOAD()> может быть полезным инструментом, хотя её непросто использовать
...
которые вы не собирались автозагружить, такие как C<DESTROY()>, деструктор
...
пространстве имён зачастую недостаток дизайна, наследование и C<AUTOLOAD()>
