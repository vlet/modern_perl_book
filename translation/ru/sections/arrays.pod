...
I<Получение> отдельного элемента массива в Perl 5 требует использования
...
скалярном контексте (при скалярном присваивании, строковой конкатенации,
сложении или в булевом контексте) возвращает количество элементов в массиве:
...
C<$#> имеет другое применение: изменение размера массива путём I<присвоения>
...
=head2 Присвоение массивам

Присвоение отдельной позиции в массиве напрямую используя индекс:
...
Для сокращения присваивания, инициализируйте массив и списка:
...
E<mdash>но помните, что эти скобки I<не> создают список. Без скобок произошло бы
...
присваивание всему массиву налагает списочный контекст.
...
потому что свежесозданные массивы создаются пустыми.

=end tip

=head2 Операции над массивами

Иногда массив удобнее использовать как упорядоченный, изменяемый набор
...
удаленияе E<mdash> опциональны; вы можете пренебречь любым из этих вариантов
поведения. Описание C<splice> в C<perlfunc> демонстрирует его равноценность с
...
стиле C. Начиная с Perl 5.12, C<each> может итерировать по массиву, используя
индекс и значение:
...
=begin programlisting
    ...
    # доступ к единственному элементу массива; I<скалярный> контекст
    $cats[-1] = get_more_cats();
...
I<Семантическая> разница огромна: срез массива всегда налагает списочный
...
элементов, сколько возможно. После присваивания C<@cats> будет содержать I<все>
аргументы, переданные в функцию. C<@dogs> будет пустым.
...
E<hellip>потому что эти скобки всего лишь группируют выражения. Они не
...
F<English.pm> E<mdash> C<$LIST_SEPARATOR>. Таким образом:
