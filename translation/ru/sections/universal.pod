...
Встроенный в Perl 5 пакет C<UNIVERSAL> E<mdash> предок всех остальных пакетов
...
истинное значение, если инвокант является названным классом или происходит от
...
I<переопределяющий> C<isa()>, обычно имеет для этого хорошую причиную.
...
соблазна; это глобальное поведение можеть иметь неочевидный побочные эффекты,
