...
предшествовать одной директиве C<else>:
...
по эстетическим причинам, как и прежнее искусство языка программирования Ada.>:
...
Условное выражение предшествует символу вопроса (C<?>), а символ двоеточия
(C<:>) разделяет альтернативы. Альтернативы E<mdash> это выражения произвольной
сложности E<mdash> включая другие тернарные условные выражения.

=begin sidebar

Интересная, хотя и малоизвестная, идиому E<mdash> использование тернарного
...
истинное значение, Perl должен будет вычислить второе подвыражение. Если первое
подвыражение вернуло ложь, то нет необходимости проверять последующие
подвыражения, так как всё выражение уже не может быть истинным:
...
булевы результаты при их вычислении, результаты других выражений E<mdash>
...
C<undef> E<mdash> преобразуется в истину в булевом контексте. Хеш, содержащий
...
в стиле I<foreach> вычисляет выражение, возвращающее список, и выполняет блок
операцию или блок, пока не израсходует весь этот список:
...
устанавливая переменную-топик C<$_> (L<default_scalar_variable>) на каждом шаге.
Perl выполняет блок для каждого целого числа и выводит их квадраты.
...
лексическуюN<E<mdash>но I<объявляйте> ваши переменные итераций как лексические,
...
итерации, так что любые модификации значения итератор сразу же изменяют исходное
значение:
...
переменной, для создания в псевдонима этой переменной в C<$_>:
...
=begin programlisting
    ...
    for ($user_input)
    {
        ...
        $_ = quotemeta; # экранировать символы, не являющиеся символами слов
    }
...
Учёт области видимости итератора в случае переменной-топика E<mdash> общий
...
Вы должны явно назначать значение итерационной переменной в конструкции цикла,
...
также на использование оператора запятая и низкоприоритетного C<and>; этот
синтаксис на удивление требователен. По возможности, предпочтительнее
использовать цикл в стиле C<foreach>, а не C<for>.
...
Цикл I<while> продолжается до тех пор, пока условие цикла возвращает ложное
...
тонкостей, связанных истинностью каждого значения.
...
быть ожидаемым поведением, но оно часто удивляет новичков.
...
C<1 while 1;>. Любое единственное выражение годится для постфиксной формы
...
циклов событий в программах с графическим интерфейсом, интерпретаторах или
сетевых серверах:
...
противном случае вероятность возникновения путаницы с переменной-топиком слишком
высока.
...
Стандартный механизм управления Perl 5 E<mdash> исключения и C<return> E<mdash>
работает, но также вы можете использовать операции I<контроля цикла>.

X<loops; C<next>>

Операция I<next> перезапускает цикл со следующей его итерации. Используйте её,
...
Операция I<last> немедленно завершает цикл. Чтобы закончить обработку файла, как
...
Операция I<redo> перезапускает текущую итерацию без повторного вычисления
...
Использование операций контроля цикла во вложенных списках может сбивать с
...
цикла, так и при преждевременном повторе итерации с помощью C<next>N<Эквивален
...
редки, но она полезна если, вам нужна гарантия того, что что-то происходит на
...
Имейте ввиду, что блок C<continut> I<не> выполянется, когда поток управления
...
В отличие от C<for>, она не итерирует по множеству. Она вычисляет своё выражение
...
несколько операций C<when> могут проверять соответствие топика выражениям,
используя семантику I<умного сравнения>. Так можно написать игру E<laquo>камень,
ножницы, бумагаE<raquo>.
...
вызов другой функции E<mdash> возвращаемое значение внутренней функции будет
