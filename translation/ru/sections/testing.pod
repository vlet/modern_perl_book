...
I<Тестирование> E<mdash> это процесс написания и запуска маленьких кусочков код,
...
запустить его, и посмотреть, как он работает. Эта I<автоматизация> чрезвычайно
...
Тестирование в Perl начинается со встроенного модуля C<Test::More> и его функции
...
становится двоичным значением. Каждое тестовое I<утверждение> E<mdash> это
простой вопрос, на который можно ответить да или нет: работает ли этот маленький
кусочек кода так, как я ожидаю? Сложные программы могут иметь тысячи отдельных
условий, и, в общем, чем мельче разбиение, тем лучше. Изолирование конкретных
поведений в отдельные утверждения позволяет вам свести к минимуму баги и
недопонимания, особенно когда вы будете модифицировать код в будущем.

Функция C<done_testing()> сообщает C<Test::More>, что программа успешно
выполнила все ожидаемые тестовые утверждения. Если программа столкнулась с
...
что что-то пошло не так. Без механизмы, подобного C<done_testing()>, как бы вы
I<узнали>? Правда, этот пример кода слишком прост, чтобы провалиться, но код,
который слишком прост, чтобы провалиться, проваливается гораздо чаще, чем можно
ожидать.
...
C<Test::More> также позволяет использовать I<план тестов> для представления
числа отдельных утверждений, которые вы планируете выполнить:
...
Вывод тестового файла, содержащего множество утверждений (особенно множество
I<проваленных> утверждений) может быть очень многословным. В большинстве случаев
вам достаточно знать либо что всё прошло, либо конкретику имеющихся провалов.
Встроенный модуль C<Test::Harness> интерпретирует TAP, и связанная с ним
...
Тут много вывода, отображающего то, что уже очевидно: второй и третий тесты
...
Алиас оболочки bash C<proveall> может оказаться полезным:
...
утомительно даёт небольшие диагностические возможности. C<Test::More>
предоставляет несколько других удобных функций утверждений.
...
C<is()> отображает ожидаемое полученное значение.
...
использую оператор C<ne>, и проходит, если они не равны. Она также налагает
...
класс (L<inheritance>) с помощью C<isa_ok()>:
...
В C<Test::More> есть ещё несколько тестовых функций, но эти наиболее полезные.
...
тестами в привязке к функционалу становится более интересной; большие тестовые
файлы сложнее поддерживать.

Кроме того, раздельные файлы тестов ускоряют разработку. Если вы добавляете
...
Последний модуль управляет планом тестов и приводит вывод тестов в TAP. Этот
дизайн позволяет множеству тестовых модулей разделять один и тот же бэкенд
...
сложные интерфейсы, I<имитируя> их (эмулирую, но производя другие результаты).
...
=item * C<Test::Database> предоставляет функции для тестирования использования
и неправильной эксплуатации баз данных. C<DBICx::TestDatabase> помогает
...
о количестве вашего кода, которое тесты действительно выполняют. В общем, чем
больше покрытие, тем лучше E<mdash> хотя стопроцентное покрытие не всегда
возможно, 95% гораздо лучше, чем 80%.

=back

См. Perl QA project (U<http://qa.perl.org/>) для большей информации о
тестировании в Perl.
