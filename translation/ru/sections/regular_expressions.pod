...
Оператор поиска соответствия (I<match>, C<m//>, сокращёно C<//>) распознаёт
регулярное выражение E<mdash> в этом примере C<hat>. Этот шабло не E<mdash>
...
Оператор соответствия регулярному выражению (C<=~>) E<mdash> инфиксный оператор
(L<fixity>), который применяет регулярное выражением, являющееся его вторым
операндом к строке, указанной в качестве первого операнда. При использовании в
скалярном контексте поиск соответствия возвращает истинное значение, если
соответствие найдено. Отрицательная форма оператора соответствия (C<!~>)
возвращает истинное значение, если соответствие не найдено.
...
как полёт на автономном боевом вертолёте в магазин на углу за сыром E<mdash> но
...
для поиска соответствия при использовании с оператором соответствия регулярному
выражению. Второй операнд E<mdash> подстрока для замены найденной части первого
операнда. Например, вылечить надоедливые летние аллергии можно так:
...
Оператор C<qr//> создаёт регулярное выражение первого класса. Для использования
интерполируйте их в оператор поиска соответствия:
...
квантификатор E<mdash> квантификатор I<ноль или один>, или C<?>:
...
В регулярном выражении любой атом, за которым следует символ C<?>, означает
E<laquo>найти ноль или одни таких атомовE<raquo>. Это регулярное выражение
проходит проверку на соответствие, если один или более символов C<a> следуют
сразу за символом C<a> и непосредственно предшествуют символу C<t>, будь это
литеральная подстрока C<cat> или C<ct>.

X<regex; one or more quantifier>
X<C<+>; one or more regex quantifier>

I<Квантификатор один или более>, или C<+>, находит соответствие только если
есть как минимум одно вхождение квантифицируемого атома:
...
атомов, который могут совпасть.

X<quantifiers; zero or more>
X<C<*>; zero or more regex quantifier>

I<Квантификатор ноль или более>, C<*>, находит соответствие нуля или более
экземпляров квантифицируемого атома:
...
Как бы глубо это ни выглядело, это позволяет вам указать необязательные
...
Квантификаторы C<+> и C<*> E<mdash> I<жадные>, они пытаются найти соответствие
настолько большой части входной строки, насколько можно. Это особенно пагубное
...
Жадные квантификаторы начинают с того, что проверяют на соответствие I<всё>, а
затем возвращаются на один символ за раз только если очевидно, что соответствия
не найдено.
...
текущее количество не проходит проверку на соответствие. Поскольку C<*> требует
...
совпадений), и к квантификаторам диапазона. В каждом случае он заставляет
регулярное выражение захватывать так мало входных данных, как возможно.
...
но ничто не заменит обширное тестирование на реальны рабочих данных.
...
I<Якоря регулярных выражений> заставляют механизм регулярных выражений начать
или закончить соответствие на абсолютной позиции. I<Якорь начала строки> (C<\A>)
...
символом слова (C<\w>) и несловесным символом (C<\W>). Используйте регулярное
...
Метасимволы C<.> означает E<laquo>найти любой символ, исключая перевод
...
использованием якорей E<mdash> для 7 по вертикали может быть C</..m/>. Конечно,
всегдя есть более чем один способ получить правильный ответ:
...
знакам пунктуации, пробельным символам и числам. Будьте определённы! Метасимвол
...
Используйте метасимвол C<\s> для поиска пробельных символов, будь это буквальный
...
символа, I<исключая> символы слов. ИСпользуйте C<\D> для поиска нецифровых
символов. Используйте C<\S> для поиска чего угодно, кроме пробельных символов.
Используйте C<\B> для поиска где угодно, кроме границы слова.
...
Если ни один из этих метасимволов недостаточно точен, определите свой
...
=begin tip Интерполяция служается
...
Используйте знак вставки (C<^>) как первый элемент класса символов чтобы сказать
...
Используйте знак вставки в любом месте, кроме первой позиции, чтобы сделать его
...
для дальнейшего использования. Извлечь из строки американские телефонные номера
...
единицы большего размера, а также захватывают части совпавших строк. Для поиска
литеральных скобок, экранируйте их с помощью обратных слешей, как видно в
C<$area_code>.
...
C<%+>. Вместо этого Perl сохраняет захваченную подстроку в серии магических
...
трудно. Именованные захваты улучшают поддерживаемость регулярных выражений
...
В списочном контексте поиск соответствия регулярному выражению возвращает список
...
Некоторым регулярным выражениям нужно найти соответствие одному варианту или
...
Метасимвол выбора указывает, что любой из предшествующих фрагментов может
совпадать. Имейте ввиду, что выбор имеет приорите (L<precedence>) даже ниже, чем
атомы:
...
=begin tip Незахваченный для вашей защиты

Преобразованное в строку регулярное выражение включает заключающую
...
проверки соответствия из источника, который вы не контролируете при написании
программы:
...
I<утверждений регулярных выражений>, которые требуют, чтобы строка удовлетворяла
определённым условиям. Эти утверждения не соответствуют отдельным символам
...
I<Утверждения нулевой ширины> соответствуют I<шаблону>. Что ещё более важно, они
при этом не потребляют часть шаблона. Например, чтобы найти кота самого по себе,
вы можете использовать утверждение границы слова:
...
использовать I<утверждение нулевой ширины отрицательного просмотра вперёд>:
...
I<Утверждение нулевой ширины положительного просмотра вперёд>:
...
для соответствия фрагмент с якорем C<.*\Z>. В противном случае захват захватил
...
Чтобы утвердить, что ваше животное никогда не появляется в начале строки, вы
можете использовать I<утверждение нулевой ширины отрицательного просмотра
назад>. Эти утверждения должны иметь фиксированный размер; вы не можете
...
можете выразить, что C<cat> всегд появляется сразу после символа пробела с
помощью I<утверждения нулевой ширины положительного просмотра назад>:
...
может быть полезен при объединении глобального соответствия регулярному
...
Новейшая возможность регулярных выражений Perl 5 E<mdash> утверждение
I<сохранения> C<\K>. Это утверждение нулевой ширины положительного просмотра
вперёд I<может> иметь переменную длину:
...
C<\K> на удивление полезно для некоторых замен, которые удаляют конец шаблона:
...
Некоторые модификаторы изменяют поведение оператора регулярного выражения. Эти
модификаторы располагаются на конце операторов поиска соответствия, замены или
C<qr//>. Например, так можно включить регистронезависимый поиск соответствия:
...
C<like()> пройдёт, потому что модификатор C</i> заставляет регулярное выражение
...
Синтаксис C<(?i)> включает регистронезависимсы поиск только внутри включаемой в
него группы: в данном случае, в именованном захвате. Вы можете использовать
...
Модификатор C</s> воспринимает исходную строку как одну линию, так что
метасимвол C<.> будет соответствовать в том числе символу переводу строки.
...
При использовании с поиском соответствия E<mdash> не заменой E<mdash> метасимвол
...
доступной строки для соответствия. Утверждения простмотра вперёд тоже могут
помочь.
...
Модификатор C</e> позволяет вам написать произвольный код Perl 5 на правой
стороне операции замены. Если совпадение будет успешным, механизм регулярных
