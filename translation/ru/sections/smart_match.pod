=head1 Умный поиск соответствия
...
Оператор умного поиска соответствия, C<~~>, сравнивает два операнда и возвращает
истинное значение, если они соответствуют. Расплывчатость определения
демонстрирует умность оператора: тип сравнения зависит от типов обоих операндов.
C<given> (L<given_when>) выполняет неявный умный поиск соответствия.

X<operators; C<~~>>
X<C<~~>; smart match operator>

Оператор умного поиска соответствия E<mdash> инфиксный оператор:
...
E<mdash> левого. Например, если правый операнд E<mdash> скаляр с числовой
составляющей, сравнение будет использовать числовой равенство. Если правый
операнд E<mdash> регулярное выражение, сравнение будет использовать grep или
соответствие шаблону. Если правый операнд E<mdash> массив, сравнение будет
выполнять grep или рекурсивный умный поиск соответствия. Если правый операнд
...
обо всех сравнениях, которые может выполнять оператор умного поиска сооветствия.

Серьёзны план для 5.16 предполагает существенное упрощение умного поиска
соответствия. Чем более сложны ваши операнды, тем больше у вас шансов получить
...
С учётом вышесказанного, умный поиск соответствия может быть полезным:
