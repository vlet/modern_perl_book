...
I<Хеш> E<mdash> структура данных первого класса в Perl, которая ставит в
соответствие строковые ключи со скалярными значениями. Так же как имя переменной
...
Хеши имеют два важных свойства: они сохраняют один скаляр на уникальный ключ и
они не предоставляют определённой сортировки ключей.
...
Присвоение хешу списка ключей и значений в одном выражении:
...
(C<< => >>) для установки соответствия значений с ключами, так как это делает
разбиение на пары более заметным на вид:
...
Присвоение хешу происходит в списочном контексте. Любая функция, вызванная в
присвоении хешу, по умолчанию будет вызвана в списочном контексте, если нет
...
C<$> для обозначения доступа к скалярному значению.
...
Унарный плюс (L<unary_coercions>) превращает то, что на что как на голое слово
...
допустимым ключом хеша. Perl пойдёт так далеко, что преобразует (L<coercion>)
...
ключей и значений. Оператор C<kesy> возвращает список ключей хеша:
...
размера хеша и случайного фактора. Несмотря на это, порядок элементов хеша
...
В скалярном контексте хеш даёт строку, представляющую соотношение полных корзин
в хеше E<mdash> внутренние детали реализации хешей, которые вы смело можете
игнорировать.

В списочном контексте хеш даёт список пар ключ/значение, подобный тому, что вы
...
Так как каждый ключ в хеше встречается только один раз, присвовение того же
...
Этот подход позволяет вам устанавливать значение по умолчанию:
...
Редко используемый встроенный модуль C<Hash::Util> предоставляет механизмы для
...
безопасности защита годится только для предотвращения случайностей; кто угодно
